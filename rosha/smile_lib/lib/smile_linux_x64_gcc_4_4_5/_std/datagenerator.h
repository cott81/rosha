...
private:
	int GenerateDiscreteData(DSL_dataGeneratorOutput &out, DSL_network &net, const std::vector<int> &outputNodes, const std::vector<char> &missing);
	int GenerateContinuousData(DSL_dataGeneratorOutput &out, DSL_network &net, const std::vector<int> &outputNodes, const std::vector<char> &missing);
...
public:
	...
	virtual int PostRecord(int recIdx) { return DSL_OKAY; }
	virtual int Close() { return DSL_OKAY; }
