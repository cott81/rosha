...
using namespace car_laser_driver_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(CarLaserDriver_QBFD_Analyzer, diagnostic_aggregator::CarLaserDriver_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

CarLaserDriver_QBFD_Analyzer::CarLaserDriver_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "CarLaserDriver_QBFD_Analyzer";
  cout << "\n\n\nCarLaserDriver_QBFD_Analyzer: const\n\n\n" << endl;
...
bool CarLaserDriver_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("car_laser_driver");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in CarLaserDriver_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in CarLaserDriver_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in CarLaserDriver_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in CarLaserDriver_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  if (systemNodesToAnalyze.size() == 0) {
    //set default
    systemNodesToAnalyze[0] = "LaserDriver";
  }
  ...
  */
  ...
  std::string path2 = path+"car_laser_driver_FailureModel_dbn.xdsl";
...
bool CarLaserDriver_QBFD_Analyzer::match(const std::string name)
{
...
bool CarLaserDriver_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool CarLaserDriver_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > CarLaserDriver_QBFD_Analyzer::report()
{
  ...
  if (this->unmatched)
  {
    ...
    bkb->SetSystemModelNodeDefaultDef("LaserDriver", 0.0, 1.0);
    ...
    kv.key = "CompFailure_LaserDriver_failure";
    kv.value = "0";
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("LaserDriver");     //node for the failure probability
