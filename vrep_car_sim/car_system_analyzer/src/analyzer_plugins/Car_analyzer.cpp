...
using namespace car_system_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(CarSystemAnalyzer, diagnostic_aggregator::CarSystemAnalyzer, diagnostic_aggregator::Analyzer)

CarSystemAnalyzer::CarSystemAnalyzer() :
    path_(""), has_initialized_(false)
{
  this->CLASSNAME = "CarSystemAnalyzer";
  cout << "\n\n\nCarSystemAnalyzer: const\n\n\n" << endl;
...
bool CarSystemAnalyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("car_system_analyzer");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in CarSystemAnalyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in CarSystemAnalyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in CarSystemAnalyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in CarSystemAnalyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
...
//  boost::shared_ptr<StatusItem> item(new StatusItem(this->itemName));
  ...
  std::string path2 = path+"Car_model.xdsl";
...
bool CarSystemAnalyzer::match(const std::string name)
{
  bool matched = false;
...
bool CarSystemAnalyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
  return false;
}

vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > CarSystemAnalyzer::report()
{
  mutexObj.lock();
  ...
  for (iter = this->reasoningResults->begin(); iter !=this->reasoningResults->end(); iter++) {
    ...
    ss << this->robotname << "_" << iter->GetNodeName();
    ss >> ds->name;
