...
using namespace car_slam_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(CarSlamRemote_QBFD_Analyzer, car_slam_analyzer_plugins::CarSlamRemote_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

CarSlamRemote_QBFD_Analyzer::CarSlamRemote_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "CarSlamRemote_QBFD_Analyzer";
  cout << "\n\n\n CarSlamRemote_QBFD_Analyzer: const\n\n\n" << endl;
  ...
  */
  ...
  this->cpuNodeStateLimits[1] = 250;    //upper threshhold


  this->memNodeStateLimits[0] = 10.0;    //low bound [kB]
  this->memNodeStateLimits[1] = 13000.0;   //upper bound
  ...
  this->streamNodeStateLimits_nl[0] = 290.0;    //lower bound old: 8
  this->streamNodeStateLimits_nl[1] = 310.0;   //upper bound old: 10.0
...
bool CarSlamRemote_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ...
  {
    ROS_WARN ("No path parameter was specified in CarSlamRemote_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in CarSlamRemote_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in CarSlamRemote_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in CarSlamRemote_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  if (systemNodesToAnalyze.size() == 0) {
    //set default
    systemNodesToAnalyze[0] = "SLAM_Remote";
  }
  ...
  */
  ...
  std::string path2 = path+"car_slam_remote_FailureModel_dbn.xdsl"; //dbn for the fdd eval experiment
...
bool CarSlamRemote_QBFD_Analyzer::match(const std::string name)
{
...
bool CarSlamRemote_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool CarSlamRemote_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > CarSlamRemote_QBFD_Analyzer::report()
{
  ...
  if (this->unmatched)
  {
    ...
    bkb->SetSystemModelNodeDefaultDef("SLAM_Remote", 0.0, 1.0);
    ...
    kv.key = "CompFailure_SLAM_Remote_failure";
    kv.value = "0";
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("SLAM_Remote");     //node for the failure probability
