...
//PLUGINLIB_REGISTER_CLASS(CarSlam_QBFD_Analyzer, vrep_slam_analyzer_plugins::CarSlam_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

CarSlam_QBFD_Analyzer::CarSlam_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "CarSlam_QBFD_Analyzer";
  cout << "\n\n\n CarSlam_QBFD_Analyzer: const\n\n\n" << endl;
...
bool CarSlam_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ...
  {
    ROS_WARN ("No path parameter was specified in CarSlam_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in CarSlam_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in CarSlam_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in CarSlam_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"car_slam_CSFailureModel_dbn.xdsl"; //dbn for the fdd eval experiment
...
bool CarSlam_QBFD_Analyzer::match(const std::string name)
{
...
bool CarSlam_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool CarSlam_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > CarSlam_QBFD_Analyzer::report()
{
  ...
  if (this->unmatched)
  {
    ...
    kv.key = "CompFailure_SLAM_failure";
    kv.value = "0";
