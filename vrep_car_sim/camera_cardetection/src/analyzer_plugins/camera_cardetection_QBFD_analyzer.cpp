...
using namespace camera_cardetection_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(Camera_CarDetection_QBFD_Analyzer, diagnostic_aggregator::Camera_CarDetection_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

Camera_CarDetection_QBFD_Analyzer::Camera_CarDetection_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "Camera_CarDetection_QBFD_Analyzer";
  cout << "\n\n\nCamera_CarDetection_QBFD_Analyzer: const\n\n\n" << endl;
...
bool Camera_CarDetection_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("camera_cardetection");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in Camera_CarDetection_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in Camera_CarDetection_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in Camera_CarDetection_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in Camera_CarDetection_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  if (systemNodesToAnalyze.size() == 0) {
    //set default
    systemNodesToAnalyze[0] = "Cam_Car";
  }
  ...
  */
  ...
  std::string path2 = path+"camera_cardetection_FailureModel_dbn.xdsl";
...
bool Camera_CarDetection_QBFD_Analyzer::match(const std::string name)
{
...
bool Camera_CarDetection_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool Camera_CarDetection_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > Camera_CarDetection_QBFD_Analyzer::report()
{
  ...
  if (this->unmatched)
  {
    ...
    bkb->SetSystemModelNodeDefaultDef("Cam_Car", 0.0, 1.0);
    ...
    kv.key = "CompFailure_Cam_failure";
    kv.value = "0";
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("Cam_Car");     //node for the failure probability
