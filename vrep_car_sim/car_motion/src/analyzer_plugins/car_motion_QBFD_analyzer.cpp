...
using namespace car_motion_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(Car_Motion_QBFD_Analyzer, diagnostic_aggregator::Car_Motion_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

Car_Motion_QBFD_Analyzer::Car_Motion_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "Car_Motion_QBFD_Analyzer";
  cout << "\n\n\nCar_Motion_QBFD_Analyzer: const\n\n\n" << endl;
  ...
  */
  ...
  this->cpuNodeStateLimits[1] = 500;    //upper threshhold
...
bool Car_Motion_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("car_motion");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in Car_Motion_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in Car_Motion_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in Car_Motion_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in Car_Motion_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"car_motion_FailureModel_dbn.xdsl";
...
bool Car_Motion_QBFD_Analyzer::match(const std::string name)
{
...
bool Car_Motion_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool Car_Motion_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > Car_Motion_QBFD_Analyzer::report()
{
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("Motion");     //node for the failure probability
