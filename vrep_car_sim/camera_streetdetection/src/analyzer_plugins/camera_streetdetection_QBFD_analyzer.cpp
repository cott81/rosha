...
using namespace camera_streetdetection_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(Camera_StreetDetection_QBFD_Analyzer, diagnostic_aggregator::Camera_StreetDetection_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

Camera_StreetDetection_QBFD_Analyzer::Camera_StreetDetection_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "Camera_StreetDetection_QBFD_Analyzer";
  cout << "\n\n\nCamera_StreetDetection_QBFD_Analyzer: const\n\n\n" << endl;
...
bool Camera_StreetDetection_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("camera_streetdetection");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in Camera_StreetDetection_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in Camera_StreetDetection_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in Camera_StreetDetection_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in Camera_StreetDetection_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"camera_streetdetection_FailureModel_dbn.xdsl";
...
bool Camera_StreetDetection_QBFD_Analyzer::match(const std::string name)
{
...
bool Camera_StreetDetection_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool Camera_StreetDetection_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > Camera_StreetDetection_QBFD_Analyzer::report()
{
