...
using namespace car_inertia_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(CarInertia_QBFD_Analyzer, diagnostic_aggregator::CarInertia_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

CarInertia_QBFD_Analyzer::CarInertia_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "CarInertia_QBFD_Analyzer";
  cout << "\n\n\nCarInertia_QBFD_Analyzer: const\n\n\n" << endl;
...
bool CarInertia_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("car_inertia");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in CarInertia_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in CarInertia_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in CarInertia_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in CarInertia_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  if (systemNodesToAnalyze.size() == 0) {
    //set default
    systemNodesToAnalyze[0] = "Inertia";
  }
  ...
  */
  ...
  std::string path2 = path+"car_inertia_FailureModel_dbn.xdsl";
...
bool CarInertia_QBFD_Analyzer::match(const std::string name)
{
...
bool CarInertia_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool CarInertia_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > CarInertia_QBFD_Analyzer::report()
{
  ...
  if (this->unmatched)
  {
    ...
    bkb->SetSystemModelNodeDefaultDef("Inertia", 0.0, 1.0);
    ...
    kv.key = "CompFailure_Inertia_failure";
    kv.value = "0";
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("Inertia");     //node for the failure probability
