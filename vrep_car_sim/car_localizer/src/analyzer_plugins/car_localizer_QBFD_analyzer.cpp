...
using namespace car_localizer_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(Car_Localizer_QBFD_Analyzer, diagnostic_aggregator::Car_Localizer_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

Car_Localizer_QBFD_Analyzer::Car_Localizer_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "Car_Localizer_QBFD_Analyzer";
  cout << "\n\n\nCar_Localizer_QBFD_Analyzer: const\n\n\n" << endl;
...
bool Car_Localizer_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ...
  {
    ROS_WARN ("No path parameter was specified in Car_Localizer_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in Car_Localizer_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in Car_Localizer_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in Car_Localizer_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"car_localizer_FailureModel_dbn.xdsl";
...
bool Car_Localizer_QBFD_Analyzer::match(const std::string name)
{
...
bool Car_Localizer_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool Car_Localizer_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > Car_Localizer_QBFD_Analyzer::report()
{
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("GPS");     //node for the failure probability
