...
using namespace vrep_localizer_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(VrepLocalizer_QBFD_Analyzer, diagnostic_aggregator::VrepLocalizer_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

VrepLocalizer_QBFD_Analyzer::VrepLocalizer_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "VrepLocalizer_QBFD_Analyzer";
  cout << "\n\n\VrepLocalizer_QBFD_Analyzer: const\n\n\n" << endl;
...
bool VrepLocalizer_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ...
  {
    ROS_WARN ("No path parameter was specified in VrepLocalizer_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in VrepLocalizer_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in VrepLocalizer_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in VrepLocalizer_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"vrep_localizer_FailureModel_dbn.xdsl"; //dbn for the fdd eval experiment
...
bool VrepLocalizer_QBFD_Analyzer::match(const std::string name)
{
