...
//PLUGINLIB_REGISTER_CLASS(VrepSlamAdapter_QBFD_Analyzer, vrep_slam_analyzer_plugins::VrepSlamAdapter_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

VrepSlamAdapter_QBFD_Analyzer::VrepSlamAdapter_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "VrepSlamAdapter_QBFD_Analyzer";
  cout << "\n\n\n VrepSlamAdapter_QBFD_Analyzer: const\n\n\n" << endl;
...
bool VrepSlamAdapter_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ...
  {
    ROS_WARN ("No path parameter was specified in VrepSlamAdapter_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in VrepSlamAdapter_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in VrepSlamAdapter_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in VrepSlamAdapter_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"vrep_slam_adapter_FailureModel_dbn.xdsl"; //dbn for the fdd eval experiment
...
bool VrepSlamAdapter_QBFD_Analyzer::match(const std::string name)
{
...
bool VrepSlamAdapter_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool VrepSlamAdapter_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > VrepSlamAdapter_QBFD_Analyzer::report()
{
  ...
  if (this->unmatched)
  {
    ...
    bkb->SetSystemModelNodeDefaultDef("SLAM_Adapter", 0.0, 1.0);
    ...
    kv.key = "CompFailure_SLAM_Adapter_failure";
    kv.value = "0";
  ...
  //  modelLink.SetCompModelNodeName("RoboPkgCS");
    int failureDetectionNode = theNet->FindNode("SLAM_Adapter");     //node for the failure probability
