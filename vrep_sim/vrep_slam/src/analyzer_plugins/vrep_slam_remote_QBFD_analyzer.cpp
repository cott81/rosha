...
using namespace vrep_slam_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(VrepSlamRemote_QBFD_Analyzer, vrep_slam_analyzer_plugins::VrepSlamRemote_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

VrepSlamRemote_QBFD_Analyzer::VrepSlamRemote_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "VrepSlamRemote_QBFD_Analyzer";
  cout << "\n\n\n VrepSlamRemote_QBFD_Analyzer: const\n\n\n" << endl;
...
bool VrepSlamRemote_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ...
  {
    ROS_WARN ("No path parameter was specified in VrepSlamRemote_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in VrepSlamRemote_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in VrepSlamRemote_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in VrepSlamRemote_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"vrep_slam_remote_FailureModel_dbn.xdsl"; //dbn for the fdd eval experiment
...
bool VrepSlamRemote_QBFD_Analyzer::match(const std::string name)
{
...
bool VrepSlamRemote_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool VrepSlamRemote_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > VrepSlamRemote_QBFD_Analyzer::report()
{
