...
using namespace vrep_laser_driver_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(VrepLaserDriver_QBFD_Analyzer, diagnostic_aggregator::VrepLaserDriver_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

VrepLaserDriver_QBFD_Analyzer::VrepLaserDriver_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "VrepLaserDriver_QBFD_Analyzer";
  cout << "\n\n\nVrepLaserDriver_QBFD_Analyzer: const\n\n\n" << endl;
...
bool VrepLaserDriver_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("vrep_laser_driver");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in VrepLaserDriver_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in VrepLaserDriver_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in VrepLaserDriver_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in VrepLaserDriver_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
...
bool VrepLaserDriver_QBFD_Analyzer::match(const std::string name)
{
...
bool VrepLaserDriver_QBFD_Analyzer::unmatch(const std::string name)
{
...
bool VrepLaserDriver_QBFD_Analyzer::analyze(const boost::shared_ptr<StatusItem> item)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > VrepLaserDriver_QBFD_Analyzer::report()
{
