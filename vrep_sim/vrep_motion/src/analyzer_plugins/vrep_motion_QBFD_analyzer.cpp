...
using namespace vrep_motion_analyzer_plugins;
using namespace diagnostic_engine;
using namespace std;

//PLUGINLIB_REGISTER_CLASS(VrepMotion_QBFD_Analyzer, diagnostic_aggregator::VrepMotion_QBFD_Analyzer, diagnostic_aggregator::Analyzer)

VrepMotion_QBFD_Analyzer::VrepMotion_QBFD_Analyzer() :
    path_(""), has_initialized_(false)

{
  this->CLASSNAME = "VrepMotion_QBFD_Analyzer";
  cout << "\n\n\n VrepMotion_QBFD_Analyzer: const\n\n\n" << endl;
...
bool VrepMotion_QBFD_Analyzer::init(const string base_name, const ros::NodeHandle &n)
{
  ROS_INFO("%s: init(): ", this->CLASSNAME);

  std::string path = ros::package::getPath("vrep_motion");
  path = path + "/model/";
  ...
  {
    ROS_WARN ("No path parameter was specified in VrepMotion_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No cap parameter was specified in VrepMotion_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_WARN ("No func parameter was specified in VrepMotion_QBFD_Analyzer! Use no parent item.");
  } else {
  ...
  {
    ROS_ERROR(
        "No power board name was specified in VrepMotion_QBFD_Analyzer! Power board must be \"Power board 10XX\". Namespace: %s", n.getNamespace().c_str());
    return false;
  }
  ...
  */
  ...
  std::string path2 = path+"vrep_motion_FailureModel_dbn.xdsl"; //dbn for the fdd eval experiment
...
bool VrepMotion_QBFD_Analyzer::match(const std::string name)
{
...
vector<boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> > VrepMotion_QBFD_Analyzer::report()
{
  ...
  boost::shared_ptr<diagnostic_msgs::DiagnosticStatus> ds = report_item_->toStatusMsg(path_); //replaced the path in the name
